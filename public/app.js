...
import { repairStudyDocumentImport } from "./js/study-document-import-repair.js?v=study-document-import-repair-v2";
...
async function submitStudyDocumentDialog() {
  ...
  if (importKind === "english") {
    const repaired = repairStudyDocumentImport(parsed);
    if (repaired.repairs.length) {
      parsed = repaired.document;
      const input = dialog.querySelector("[data-study-document-dialog-input]");
      if (input) input.value = JSON.stringify(parsed, null, 2);
      showToast(`${repaired.repairs.length} semantic field${repaired.repairs.length === 1 ? "" : "s"} repaired automatically`);
    }
  }
